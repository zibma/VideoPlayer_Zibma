...
import 'package:newpipeextractor_dart_example/flutter_xlider.dart';
...
class _PlayerProgressBarState extends State<PlayerProgressBar>
  ...
  StreamSegment currentSegment(double value) {
    ...
    if (value < widget.segments[1].startTimeSeconds) {
      ...
      List<int> startTimes = List.generate(widget.segments.length,
          (index) => widget.segments[index].startTimeSeconds).toList();
      int closestStartTime =
          (startTimes.where((e) => e >= position).toList()..sort()).first;
      int nearIndex = (widget.segments.indexWhere(
              (element) => element.startTimeSeconds == closestStartTime)) -
          1;
              ...
              Expanded(
                child: SizedBox(
                  ...
                  child: FlutterSlider(
                    ...
                    tooltip: FlutterSliderTooltip(
                      ...
                      format: (value) {
                        ...
                        String positionString =
                            "${Duration(seconds: position).inMinutes.toString().padLeft(2, '0')}:" +
                    ...
                    values: [widget.position.inSeconds.toDouble()],
                    ...
                    onDragging: (_, value, ___) {
                      if (widget.segments != null &&
                        ...
                        if (currentLabel != currentSegment(value).title)
                          setState(
                              () => currentLabel = currentSegment(value).title);
                      }
                    },
                  ),
                ),
